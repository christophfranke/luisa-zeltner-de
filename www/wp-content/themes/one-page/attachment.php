                ...
                <div class="post_content">
                    <p>
                        <a href="<?php echo get_permalink($post->post_parent); ?>" title="<?php esc_attr(printf(RETURN_TO_S, get_the_title($post->post_parent))); ?>" rel="gallery">
                            ...
                            printf(__('<span>&larr;</span> %s', 'one-page'), get_the_title($post->post_parent));
                            ?>
                        </a>
                    </p>
                    <p>
                        <?php
                        printf(BY_AUTHOR, 'meta-prep meta-prep-author', sprintf('<a class="url fn n" href="%1$s" title="%2$s">%3$s</a>', get_author_posts_url(get_the_author_meta('ID')), sprintf(esc_attr__('View All Post By', 'one-page'), get_the_author()), get_the_author()
                        ));
                        ?>
                        <span>|</span>
                        <?php
